...
module FeatureHelpers
  ...
  def form_is_filled_in_by_form_filler(live_form_link, yes_branch: false, confirmation_email: nil, copy_of_answers: false)
    ...
    if copy_of_answers
      ...
      expect(page).to have_current_path(%r{\Ahttps://signin(?:\.integration)?\.account\.gov\.uk}, url: true)
  ...
  def check_submission(submission_reference)
    ...
    while Time.now - start_time < 60
      ...
      raise "Could not query submission status API: #{status_api_response}" unless %w[204 404].include?(status_api_response.code)
